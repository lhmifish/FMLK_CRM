...
<script type="text/javascript">
	...
	$(document).ready(
			function() {
				...
				startWeekStr = formatDate(new Date(da.getFullYear(), da
						.getMonth(), da.getDate() - DayOfWeek + 1));
				
				endWeekStr = formatDate(new Date(da.getFullYear(), da
						.getMonth(), da.getDate() + (6 - DayOfWeek + 1)));
				startWeekStr2 = formatDate(new Date(da.getFullYear(), da
						.getMonth(), da.getDate() - DayOfWeek + 1 - 7));
				endWeekStr2 = formatDate(new Date(da.getFullYear(), da
						.getMonth(), da.getDate() + (6 - DayOfWeek + 1 - 7)));
	...
	function getUserName(uid) {
		...
		$.ajax({
			...
			success : function(returndata) {
				...
				$(document).attr("title", data[0].name + "的周报");
		...
		$
				.ajax({
					...
					data : {
						"date" : startWeekStr,
						"date2" : endWeekStr,
						"lastDate" : startWeekStr2,
						"lastDate2" : endWeekStr2,
						"userName" : $("#user").val()
					},
					cache : false,
					async : false,
	...
	function createWeekReport() {
		...
		$.ajax({
			url : "${pageContext.request.contextPath}/createWeekReport",
			type : 'POST',
			cache : false,
			data : {
				"userName" : userName,
	...
	function showLastWeekUploadReport() {
		...
		window.wxc.xcConfirm(divLastWeekReport, {
			title : $("#user").val() + " " + startWeekStr2 + " 至 "
					+ endWeekStr2 + " 周总结"
		},{onOk:function(){
	...
	function showThisDailyUploadReport() {
		...
		window.wxc.xcConfirm(divThisDailyUploadReport, {
			title : $("#user").val() + " " + startWeekStr + " 至 " + endWeekStr
					+ " 日报"
		},{onOk:function(){
			 $('#mBody').css({'position':'static'});
		}});
...
<body class="body-gray" style="margin: auto;" id="mBody">
	<div class="form">
		<div class="top" style="width: 100%;margin-bottom: 50px;">
			...
			<!-- <p>上周周总结</p>
			<div class="mes3">
				<textarea id="weekReport2" style="border: 0; width: 95%"></textarea>
			</div> -->

		</div>

		<!-- <div id="list" style="width: 100%; margin-bottom: 50px;"></div> -->

	</div>

	<div class="button-submit">
		<button type="button" onclick="createWeekReport();"
