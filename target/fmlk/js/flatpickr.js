...
function Flatpickr(element, config) {
	...
	function isEnabled(dateToCheck) {
		...
		for (var i = 0; i < array.length; i++) {
			...
			if (d instanceof Function && d(dateToCheck)) // disabled by
															// function
		...
		if (typeof date === "string") {
			...
			if (date === "today") {
				...
				var m = date.match(timeRegex),
				    hours = !m[4] ? m[1] // military time, no conversion
											// needed
				: m[1] % 12 + (m[4].toLowerCase() === "p" ? 12 : 0); // am/pm

				date = new Date();
				date.setHours(hours, m[2] || 0, m[3] || 0);
			} else if (/Z$/.test(date) || /GMT$/.test(date)) // datestrings
																// w/ timezone
...
Flatpickr.defaultConfig = {
	...
	/*
	 * clicking on input opens the date(time)picker. disable if you wish to open
	 * the calendar manually with .open()
	 */
	clickOpens: true,
	...
	// noCalendar: true will hide the calendar. use for a time picker along w/
	// enableTime
	noCalendar: false,
	...
	// defaultDate - either a datestring or a date object. used for
	// datetimepicker"s initial value
	defaultDate: null,
	...
	// displays calendar inline. see
	// https://chmln.github.io/flatpickr/#inline-calendar
	inline: false,
	...
	// code for previous/next icons. this is where you put your custom icon code
	// e.g. fontawesome
