...
// reader鈥檚 list of elements (headings, form controls, landmarks, etc.) in the document.
...
var defaultInputValidators = {
  ...
  text:function text(string, extraParams) {
	  return !(/^\s*$/g).test(string) ? Promise.resolve() : Promise.reject(extraParams && extraParams.validationMessage ? extraParams.validationMessage : '输入不能为空'); 
  },
  select:function select(string, extraParams) {
	  return !(/^\s*$/g).test(string) ? Promise.resolve() : Promise.reject(extraParams && extraParams.validationMessage ? extraParams.validationMessage : '选择不能为空');
  }
};
