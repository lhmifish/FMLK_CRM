...
<script type="text/javascript">
	...
	function getMyTenderList(tPage) {
		...
		setTimeout(function() {
			get("getTenderList", params, false)
			if (requestReturn.result == "error") {
				closeLoading();
				alert(requestReturn.error);
			} else {
				var data = requestReturn.data.tenderlist;
				var str = "";
				var tenderResult;
				var tenderState;
				var num = data.length;
					...
					for ( var i in data) {
						...
						if (i >= 10 * (tPage - 1) && i <= 10 * tPage - 1) {
							...
							str += '<tr style="width:100%">'
                                ...
                                + '<td style="width:19%;font-size:14px" class="tdColor2">'
								+ data[i].tenderNum
								+ '</td>'
								+ '<td style="width:46%" class="tdColor2">'
								+ getProject("projectId",data[i].projectId).projectName
								+ '</br><span style="font-size:10px">'
								+ getCompany("companyId",data[i].tenderCompany).companyName
								+ '</span></td>'
				...
				document.getElementById('p').innerHTML = tPage + "/" + lastPage;
		        $("#tb").empty();
		        $("#tb").append(str);
	...
	function changeCompany(tCompanyId) {
		var salesId = getCompany("companyId",tCompanyId).salesId;
			...
			<div class="vip">
				<div class="conform">
					<form>
						...
						<div class="cfD">
							  <label style="margin-right: 10px; margin-left: 124px">销售人员：</label><select
								...
								placeholder="yyyy/mm/dd"> <span id="dd"></span><Strong
								style="margin-left: 15px; margin-right: 5px">至</Strong> <input
								type="text" id="date2" style="width: 116px; padding-left: 8px"
								class="input3" placeholder="yyyy/mm/dd"> <span id="dd2"></span>
						</div>

						<div class="cfD">
							<label
								style="margin-left: 124px; margin-right: 10px">投标类型：</label><select
							...
							<label style="margin-left: 48px;">投标结果：</label><select
								class="selCss" style="width: 150px" id="tenderResult">
