...
<script type="text/javascript">
	...
	function getWechatList(dept) {
		$.ajax({
			url : "${pageContext.request.contextPath}/wechatList",
			type : 'GET',
			...
			success : function(returndata) {
				...
				for ( var i in data) {
					...
					if(data[i].date==$('#date').val()){
						dateStr = '<td style="width:20%;"><a>' + data[i].date + '</a></td>';
					}else{
						dateStr = '<td style="width:20%;background:pink"><a>' + data[i].date + '</a></td>';
					}
	...
	function queryList(){
		getWechatList($('#departmentSel').val());
	}
