...
<script type="text/javascript">
	...
	function getWechatList() {
		$.ajax({
			url : "${pageContext.request.contextPath}/wechatUserList",
			type : 'GET',
			data : {"date" :  $('#date').val()},
			cache : false,
			success : function(returndata) {
				...
				var data = eval("(" + returndata + ")").dailywechatlist;
				 for ( var i in data) {
					var startTimeStr = "";
					var endTimeStr = "";
					if (data[i].startTime == "no signIn") {
						startTimeStr = '<td style="width:20%;color:red;background:yellow"><a>未签到</a></td>'
					} else if(data[i].startTime == "rest"){
						startTimeStr = '<td style="width:20%;color:red;background:pink"><a>休息</a></td>'
					}else {
						startTimeStr = '<td style="width:20%"><a>'
								+ data[i].startTime + '</a></td>'
					} 
					
					if (data[i].endTime == "no signOut") {
						endTimeStr = '<td style="width:20%;color:red;background:yellow"><a>未签退</a></td>'
					} else if(data[i].endTime == "rest"){
						endTimeStr = '<td style="width:20%;color:red;background:pink"><a>休息</a></td>'
					} else {
						endTimeStr = '<td style="width:20%"><a>'
								+ data[i].endTime + '</a></td>'
					} 
					...
					str += '<tr style="color: #000;width: 100%;">'
							...
							+ '<td style="width:20%;"><a>' + data[i].name + '</a></td>'
							+ startTimeStr 
							+ endTimeStr
							+ '<td style="width:20%"><a>' + "1111" + '</a></td>'
							+ '</tr>';
                }
				$("#tb").empty();
				$("#tb").append(str); 
		...
		<div id="div1"
			...
			<table style="width: 100%; text-align: center;">
				<tr style="width: 100%;">
					...
					<td style="width: 20%">上班时间</td>
					<td style="width: 20%">下班时间</td>
